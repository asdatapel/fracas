...
def get_bone_weights(obj):
    armature = bpy.data.objects['Armature'].pose.bones

    bones = {}
    for i, b in enumerate(armature):
        bones[b] = i
    ...
    for bone in armature:
        ...
        for v in bone_verts:
            for g in v.groups:
               if g.group == gidx: 
                    if not v.index in vertex_bone_weights:
                        vertex_bone_weights[v.index] = []    
                    w = g.weight
                    vertex_bone_weights[v.index].append([bones[bone], w])
...
def to_fmesh(obj):
    ...
    bone_weights = get_bone_weights(obj)
    
    components = [3, 2, 3, 3, 3, 4, 4] + [2 for i in range(len(mesh.uv_layers) - 1)]
    
    data = array('f')
    data.append(VERSION)
    data.append(7 + (len(mesh.uv_layers) - 1)) # num components
    ...
    for i, tri in enumerate(mesh.loop_triangles):
        for i, (vert_idx, loop_idx) in enumerate(zip(tri.vertices, tri.loops)):
            ...
            for i in range(4):
                if vert_idx in bone_weights and i < len(bone_weights[vert_idx]):
                    data.append(bone_weights[vert_idx][i][0])
                else:
                    data.append(0)
                    
            for i in range(4):
                if vert_idx in bone_weights and i < len(bone_weights[vert_idx]):
                    data.append(bone_weights[vert_idx][i][1])
                else:
                    data.append(0)
