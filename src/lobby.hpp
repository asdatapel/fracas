...
struct Lobby {
  ...
  Lobby() = default;
  Lobby(GameProperties properties) { this->properties = properties; }
  ...
  void remove_player(ClientId client_id, Broadcaster broadcaster)
  {
    ...
    if (stage != LobbyStage::NOT_STARTED &&
        (game.num_players(0) == 0 || game.num_players(1) == 0)) {
      end_game(broadcaster);
    }
  ...
  bool ready_to_start()
  {
    if (stage != LobbyStage::NOT_STARTED) return false;

    int fam_0_count = game.num_players(0);
    int fam_1_count = game.num_players(1);
    return fam_0_count > 0 && fam_0_count < MAX_PLAYERS_PER_GAME / 2 && fam_1_count > 0 &&
           fam_1_count < MAX_PLAYERS_PER_GAME / 2;
  }

  void start_game(Broadcaster broadcaster, GameId game_id)
  {
    // game = GameState(); TODO this resets players which are now stored in GameState. Either remove
    // them from GameState or make a GameState::reset function
    stage = LobbyStage::IN_GAME;

    for (int i = 0; i < game.players.len; i++) {
      Peer *peer = &broadcaster.rpc_server->server_data->clients.at(game.players[i].id).peer;
      ((RpcServer *)broadcaster.rpc_server)
          ->GameStarted(peer, GameStartedMessage{game_id, game.players[i].id});
    }

    set_next_stage(&Lobby::stage_start_round);
  }

  bool has_started() { return game.round >= 0; }
  ...
  void do_next_stage(Broadcaster broadcaster) { (this->*next_stage)(broadcaster); }
  ...
  void stage_start_faceoff(Broadcaster broadcaster)
  {
    ...
    broadcaster.broadcast(&RpcServer::InGameStartFaceoff,
                          InGameStartFaceoffMessage{faceoffers.first->id, faceoffers.second->id});
  ...
  void stage_start_play(Broadcaster broadcaster)
  {
    ...
    broadcaster.broadcast(&RpcServer::InGameStartPlay,
                          InGameStartPlayMessage{game.faceoff_winning_family});
    set_next_stage(&Lobby::stage_prep_for_prompt_for_answer);
  }
  ...
  void stage_start_steal(Broadcaster broadcaster)
  {
    ...
    broadcaster.broadcast(&RpcServer::InGameStartSteal,
                          InGameStartStealMessage{1 - game.faceoff_winning_family});
    set_next_stage(&Lobby::stage_prep_for_prompt_for_answer);
  }
  ...
  void stage_end_game(Broadcaster broadcaster) { end_game(broadcaster); }
  ...
  void waiter_pass_or_play(Broadcaster broadcaster, uint64_t elapsed_micros)
  {
    ...
    if (waiter_elapsed >= waiter_deadline) {
      ...
      broadcaster.broadcast(&RpcServer::InGamePlayerChosePassOrPlay,
                            InGameChoosePassOrPlayMessage{true});
...
void RpcServer::HandleGetGame(ClientId client_id, GetGameRequest *req, GetGameResponse *resp)
{
  ...
  for (int i = 0; i < lobby->game.players.len; i++) {
    resp->players.push_back({lobby->game.players[i].id, lobby->game.players[i].name,
                             lobby->game.players[i].family == 1});
  }
}

void RpcServer::HandleCreateGame(ClientId client_id, CreateGameRequest *req,
...
void RpcServer::HandleInGameBuzz(ClientId client_id, Empty *req, Empty *resp)
{
  ...
  Broadcaster{this, lobby}.broadcast(
      &RpcServer::InGamePlayerBuzzed,
      InGamePlayerBuzzedMessage{client_id, lobby->game.buzzing_family});
...
void RpcServer::HandleInGameChoosePassOrPlay(ClientId client_id, InGameChoosePassOrPlayMessage *req,
                                             Empty *resp)
{
  ...
  if (req->play) {
    ...
    Broadcaster{this, lobby}.broadcast(&RpcServer::InGamePlayerChosePassOrPlay,
                                       InGameChoosePassOrPlayMessage{true});
  } else {
    lobby->game.playing_family = 1 - lobby->game.faceoff_winning_family;
    Broadcaster{this, lobby}.broadcast(&RpcServer::InGamePlayerChosePassOrPlay,
                                       InGameChoosePassOrPlayMessage{false});
  }
