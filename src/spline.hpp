...
void debug_draw_sline(RenderTarget target, Vec2f start, Vec2f end, Color color = {.5, .2, .8, 1},
                      float thickness = 3)
{
  ...
  Vec2f v1 = {start.x - cross.x, start.y - cross.y}, v2 = {start.x + cross.x, start.y + cross.y},
        v3 = {end.x + cross.x, end.y + cross.y}, v4 = {end.x - cross.x, end.y - cross.y};
...
float catmull_rom(float t, float v0, float v1, float v2, float v3)
{
  ...
  return 0.5 * ((2 * v1) + (-v0 + v2) * t + (2 * v0 - 5 * v1 + 4 * v2 - v3) * t2 +
                (-v0 + 3 * v1 - 3 * v2 + v3) * t3);
}
...
float catmull_rom_tangent(float t, float v0, float v1, float v2, float v3)
{
  ...
  return 0.5 * ((-v0 + v2) + (2 * v0 - 5 * v1 + 4 * v2 - v3) * 2 * t +
...
float constant_distance_t(Spline3 spline, float t)
{
  auto get_segment = [&](int i) {
    return Segment{spline.points[i - 1], spline.points[i], spline.points[i + 1],
                   spline.points[i + 2]};
  };
...
void draw_spline(Spline3 &spline, RenderTarget target, InputState *input, Memory mem,
                 Camera *camera, bool debug)
{
  ...
  if (debug) {
    ...
    for (int p_i = 1; p_i < spline.points.len - 2; p_i++) {
      Vec3f p0 = spline.points[p_i - 1], p1 = spline.points[p_i], p2 = spline.points[p_i + 1],
            p3 = spline.points[p_i + 2];
      for (int i = 1; i <= 100; i++) {
        float t               = i / 100.f;
        Vec3f line_end        = {catmull_rom(t, p0.x, p1.x, p2.x, p3.x),
    ...
    while (t < spline.points.len - 2) {
      ...
      Vec3f p0 = spline.points[anim_point - 1], p1 = spline.points[anim_point],
            p2 = spline.points[anim_point + 1], p3 = spline.points[anim_point + 2];

      Vec3f anim_pos = {catmull_rom(t - anim_point, p0.x, p1.x, p2.x, p3.x),
                        catmull_rom(t - anim_point, p0.y, p1.y, p2.y, p3.y),
                        catmull_rom(t - anim_point, p0.z, p1.z, p2.z, p3.z)};
      Vec3f tangent  = {catmull_rom_tangent(t - anim_point, p0.x, p1.x, p2.x, p3.x),
                       catmull_rom_tangent(t - anim_point, p0.y, p1.y, p2.y, p3.y),
                       catmull_rom_tangent(t - anim_point, p0.z, p1.z, p2.z, p3.z)};

      t += fmax(0.001,
                1 / sqrt(tangent.x * tangent.x + tangent.y * tangent.y + tangent.z * tangent.z));

      Vec3f cross    = normalize(Vec3f{tangent.z, tangent.z, -tangent.x - tangent.y});
      Vec3f cross_p1 = {anim_pos.x - cross.x / 10.f, anim_pos.y - cross.y / 10.f,
                        anim_pos.z - cross.z / 10.f};
      Vec3f cross_p2 = {anim_pos.x + cross.x / 10.f, anim_pos.y + cross.y / 10.f,
                        anim_pos.z + cross.z / 10.f};
