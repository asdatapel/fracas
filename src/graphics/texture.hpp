...
struct Texture2D : Texture
{
              ...
              bool want_mipmaps = true)
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, gl_internalformat, width, height, 0, gl_format, GL_UNSIGNED_BYTE, 0);

        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, want_mipmaps ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
    ...
    void upload(void *data, GLenum pixel_data_type, bool want_mipmaps)
    {
        ...
        auto [gl_internalformat, gl_format] = format_to_opengl(format);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, gl_format, pixel_data_type, (void *)data);
    ...
    void upload(float *data, bool want_mipmaps = false)
    {
        upload(data, GL_FLOAT, want_mipmaps);
    }
};
...
struct Cubemap : public Texture
{
            ...
            bool want_mipmaps = true)
    {
        ...
        {
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, gl_internalformat, width, height, 0, gl_format, GL_UNSIGNED_BYTE, nullptr);
        }

        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, want_mipmaps ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
