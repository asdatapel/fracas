...
struct Bloomer {
  ...
  Bloomer(uint32_t width, uint32_t height)
  {
    for (int layer = 0; layer < N_LAYERS; layer++) {
      horizontal_blur_targets[layer] =
          RenderTarget(width / pow(2, layer + 1), height / pow(2, layer), TextureFormat::RGB16F,
                       TextureFormat::NONE);
      vertical_blur_targets[layer] =
          RenderTarget(width / pow(2, layer + 1), height / pow(2, layer + 1), TextureFormat::RGB16F,
                       TextureFormat::NONE);
      up_targets[layer] = RenderTarget(width / pow(2, layer + 1), height / pow(2, layer + 1),
                                       TextureFormat::RGB16F, TextureFormat::NONE);
    }
  }
  ...
  RenderTarget get_final() { return up_targets[0]; }
};