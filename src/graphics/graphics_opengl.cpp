...
RenderTarget init_graphics(uint32_t width, uint32_t height)
{
  ...
  single_channel_font_shader =
      load_shader(create_shader_program("resources/shaders/single_channel_font"));
  ...
  add_shader             = load_shader(create_shader_program("resources/shaders/add"));
  threed_with_planar_shader =
      load_shader(create_shader_program("resources2/shaders/threed_with_planar"));
  threed_with_normals_shader =
      load_shader(create_shader_program("resources2/shaders/threed_with_overlay_normals"));
...
void bind_shader(Shader shader) { glUseProgram(shader.shader_handle); }
...
{
  auto to_gl         = [](float c, float d) { return c / (d / 2) - 1; };
  float vert_data[8] = {
      to_gl(v1.x, target.width),   -to_gl(v1.y, target.height), to_gl(v2.x, target.width),
      -to_gl(v2.y, target.height), to_gl(v4.x, target.width),   -to_gl(v4.y, target.height),
      to_gl(v3.x, target.width),   -to_gl(v3.y, target.height),
  };
...
{
  Vec2f v1 = {rect.x, rect.y}, v2 = {rect.x + rect.width, rect.y},
        v3 = {rect.x + rect.width, rect.y + rect.height}, v4 = {rect.x, rect.y + rect.height};
