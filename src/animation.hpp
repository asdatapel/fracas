...
struct Bone {
  ...
  {
    auto lerp_quat = [](glm::quat a, glm::quat b, float t) { return (1 - t) * a + t * b; };
    ...
    return glm::translate(glm::mat4(1.0), {pos.x, pos.y, pos.z}) * glm::toMat4(rot) *
           glm::scale(glm::mat4(1.0), {scale.x, scale.y, scale.z});
  }

  int num_frames() { return position_keys.len; }
...
Animation parse_animation(FileData file, Memory mem)
{
  ...
  for (int bone_index = 0; bone_index < animation.num_bones; bone_index++) {
    ...
    data += sizeof(int);
    b->rotation_keys.data =
        (glm::quat *)mem.allocator->alloc(sizeof(glm::quat) * b->rotation_keys.len);
