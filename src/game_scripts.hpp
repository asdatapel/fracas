...
struct PlayerChosePassOrPlaySequence : Sequence
{
    ...
    bool update(float timestep, Scenes scenes, ClientGameData *game_data, InputState *input_state, RpcClient *rpc_client)
    {
        ...
        step({
            scenes.ui_controller->popup_banner(play ? "\"Play\"" : "\"Pass\"");
        })
...
struct EndRoundSequence : Sequence
{
    ...
    bool update(float timestep, Scenes scenes, ClientGameData *game_data, InputState *input_state, RpcClient *rpc_client)
    {
        ...
        if (round_winner == 0) {
            step({
                scenes.main->set_sequence(scenes.assets->get_keyed_animation("resources/test/family0_round_win"));
                scenes.main->play_sequence();
                scenes.main->set_t(0);
            })
        } else if (round_winner == 1) {
            step({
                scenes.main->set_sequence(scenes.assets->get_keyed_animation("resources/test/family1_round_win"));
                scenes.main->play_sequence();
                scenes.main->set_t(0);
            })
        } else {
...
struct EndGameSequence : Sequence
{
    ...
    bool update(float timestep, Scenes scenes, ClientGameData *game_data, InputState *input_state, RpcClient *rpc_client)
    {
        if (game_winner == 0) {
            step ({
                scenes.main->set_sequence(scenes.assets->get_keyed_animation("resources/test/family0_game_win"));
                scenes.main->play_sequence();
                scenes.main->set_t(0);
            })
        } else {
