...
struct Selectable {
  ...
  static Selectable check(InputState *input, Rect rect, Selectable prev = {}, bool exclusive = true,
                          bool on_up = true, Rect mask = {})
  {
    Selectable ret    = prev;
...
{
  return {1920 - dimensions.x - distance.x, 1080 - dimensions.y - distance.y, dimensions.x,
          dimensions.y};
}

Rect anchor_top_right(Vec2f dimensions, Vec2f distance)
{
  return {1920 - dimensions.x - distance.x, distance.y, dimensions.x, dimensions.y};
}

Rect translate(Rect rect, Vec2f d) { return {rect.x + d.x, rect.y + d.y, rect.width, rect.height}; }

Rect add_border(Rect rect, Vec2f border)
{
  return {rect.x + border.x, rect.y + border.y, rect.width - (border.x * 2.f),
          rect.height - (border.y * 2.f)};
}

Rect scale(Rect rect, Vec2f f)
{
  return {rect.x + ((1 - f.x) / 2) * rect.width, rect.y + ((1 - f.y) / 2) * rect.height,
          rect.width * f.x, rect.height * f.y};
}
...
struct Button2 {
  ...
  bool update_and_draw(RenderTarget target, InputState *input, Font *font)
  {
    ...
    draw_text_cropped(*font, target, text, actual_rect.x + text_border_x,
                      (actual_rect.y + actual_rect.height) - text_border_y - text_height,
...
struct TextBox2 {
  ...
  void update_and_draw(RenderTarget target, InputState *input, Font *font)
  {
    ...
    draw_rect(target,
              {text_x, text_y + (font->baseline * text_scale) + text_border / 2.f, max_text_width,
               text_border / 2.f},
              darken(color, .1f));
...
struct CheckBox {
  ...
  bool update_and_draw(RenderTarget target, InputState *input, Font *font)
  {
    ...
    selectable = Selectable::check(input, rect, selectable);
    if (selectable.just_selected) checked = !checked;
    if (checked) draw_rect(target, scale(rect, {.75f, .75f}), selected_color);
...
struct List : SelectionGroup {
  ...
  void update_and_draw(RenderTarget target, InputState *input, Font *font)
  {
    ...
    if (max_items <= 0) {
      ...
      Rect scrollbar_rect  = {rect.x + item_width + vertical_border, rect.y + scrollbar_y_offset,
                             scrollbar_width, scrollbar_height};
      scrollbar_selectable = Selectable::check(input, scrollbar_rect, scrollbar_selectable);
      if (scrollbar_selectable.focus_started) scrollbar_y_offset += input->mouse_pos_delta.y;
      for (int i = 0; i < input->key_input.len; i++) {
      ...
      if (scrollbar_y_offset < 0) scrollbar_y_offset = 0;
      if (scrollbar_y_offset > scrollbar_max_y_offset) scrollbar_y_offset = scrollbar_max_y_offset;
    ...
    Rect top_item_rect = {rect.x, rect.y - (total_item_height * scrollbar_percentage), item_width,
                          ITEM_HEIGHT};
...
struct MenuPage {
  virtual void update_and_draw(RenderTarget target, InputState *input, MainMenu *menu,
                               ClientGameData *game_data) = 0;
};
...
struct MainPage : MenuPage {
  ...
  void update_and_draw(RenderTarget target, InputState *input, MainMenu *menu,
                       ClientGameData *game_data) override
  {
    {
      ...
      draw_text_cropped(*title_font, target, title_text_2, 25.f, 25.f + 15 + line1_height, scale,
                        scale);
    }
...
struct LobbyPage : MenuPage {
  ...
  LobbyPage(Assets *assets, RpcClient *rpc_client, Memory mem)
  {
    ...
    swap_button.rect =
        anchor_top_right({0.2f * 1920, 0.2f * 1920 * 9.f / 16.f / 2.f}, {25.f, 25.f});
    ...
    if (game_owner_id > 0) {
      if (player_selection_group.selected) {
        if (swap_button.update_and_draw(target, input, font)) {
          rpc_client->SwapTeam(
              {game_id, ((List *)player_selection_group.selected)->get_selected_id()});
        }
      }
...
struct JoinGamePage : MenuPage {
  ...
  JoinGamePage(Assets *assets, RpcClient *rpc_client, Memory mem)
  {
    ...
    game_list_panel = {title_background.x, title_background.y + title_background.height + 25.f,
...
struct CreateGamePage : MenuPage {
  ...
  CreateGamePage(Assets *assets, RpcClient *rpc_client, Memory mem)
  {
    ...
    game_settings_panel = {title_background.x, title_background.y + title_background.height + 25.f,
                           title_background.width,
                           1080 - (title_background.y + title_background.height + 25.f) - 25.f};
    ...
    game_name_label = {
        {game_settings_content.x, game_settings_content.y, game_settings_content.width, 40.f},
        "Game Name:"};
    game_name_text_box.rect =
        translate(game_name_label.rect, {0, game_name_label.rect.height + 10.f});
    game_name_text_box.rect.height = 70.f;

    user_name_label.rect =
        translate(game_name_text_box.rect, {0, game_name_text_box.rect.height + 10.f});
    user_name_label.rect.height = 40.f;
    user_name_label.text        = "Your Name:";
    user_name_text_box.rect =
        translate(user_name_label.rect, {0, user_name_label.rect.height + 10.f});
...
struct SettingsPage : MenuPage {
  ...
  SettingsPage(Assets *assets, RpcClient *rpc_client, Memory mem)
  {
    ...
    Rect game_settings_content          = add_border(game_settings_panel, {25.f, 25.f});
    server_ip_address_text_box.rect     = {game_settings_content.x, game_settings_content.y,
                                       game_settings_content.width, 75.f};
    connect_to_server_button.rect       = translate(server_ip_address_text_box.rect,
                                              {0, server_ip_address_text_box.rect.height + 5.f});
